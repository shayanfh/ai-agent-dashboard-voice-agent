...
class OpenAICallAnalyzer:
    ...
    _instructions = (
        ...
        "summary in the predominant language of the conversation and classify exactly one "
        "outcome. Apply the same meaning-based rules in every language, especially Arabic, "
        "Persian, and English. Be permissive when identifying a completed booking: once the "
        "conversation clearly concerns making a booking, reservation, or order, use "
        "booking_created when the caller asks to make it, supplies booking details, accepts the "
        "proposed details, or responds with a positive or neutral acknowledgement. In that "
        "booking context, acknowledgements such as Arabic 'نعم', 'تمام', 'طيب', 'ماشي', 'أوكي', "
        "or 'إن شاء الله', Persian 'بله', 'باشه', or 'اوکی', and English 'yes', 'okay', or 'sure' "
        "normally count as confirmation. If the assistant states that the booking was completed "
        "and the caller does not reject or correct it, prefer booking_created. Do not use "
        "booking_created when the caller explicitly cancels, refuses, postpones the decision, "
        "only asks for information or availability without requesting a booking, or when a "
        "technical or operational failure prevents completion. Use "
        ...
        "the transcript. Use empty strings and an empty facts list when data is absent."
    )
